...
IMAGE_FLOPSAR="flopsar/flopsar:2.1"
...
function dock_start_ecomm {
        ...
        docker run -t --net $BRIDGE -p 8780:8780 --name $CONTAINER_ECOMM -e FLOPSAR_MANAGER=$CONTAINER_FLOPSAR -d $IMAGE_ECOMM /start-flopsar.sh
}
